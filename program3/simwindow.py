...
from program3.button import Button
...
class SimWindow(object):
    def __init__(self, logic):
        ...
        self.logic = logic
        #  TEMP
        # self.logic.read_file('inputs/test1.data')
        ...
        self.button_back = Button((850, 400, 200, 50), self.dark_grey, self.step_backward, text="Step Backward")
    ...
    def draw_arrow(self, start, end, type):
        if type == "HOLD":
    ...
    def draw_resource_process(self):
        ...
        for i in range(1, num_r + 1):
            ...
            self.location_r.append((int(i * ((self.width - self.menu_width) / (num_r + 1)) - (self.shape_size / 2)), self.shape_size))
            pygame.draw.rect(
                self.screen,
                self.resource_color,
                (self.location_r[i - 1][0],
                 ...
                 self.shape_size))
            self.render_text(self.font_pr, "R" + str(i - 1), (self.location_r[i - 1][0] + r_xpad, self.location_r[i - 1][1] + r_ypad), self.dark_blue)
        ...
        for i in range(1, num_p + 1):
            ...
            self.render_text(self.font_pr, "P" + str(i - 1), (self.location_p[i - 1][0] + p_xpad, self.location_p[i - 1][1] + p_ypad), self.dark_green)

    def render_text(self, font, text, xy_location, color):
    ...
    def step_forward(self):
        ...
        self.status_text1 = self.font_menu.render("Step " + str(self.logic.get_state_num()) + " of " + str(len(self.logic.get_steps())), False, self.black)
        self.screen.blit(self.status_text1, (820, 60))

        self.status_text2 = self.font_menu.render("Processes: " + str(num_p) + "  Resources: " + str(num_r), False, self.black)
        self.screen.blit(self.status_text2, (820, 85))
        ...
        for resource in range(num_r):
            for process in range(num_p):
                if holders[resource][process]:
                    ...
                    self.draw_arrow((self.location_p[process][0], self.location_p[process][1] + p_padh), (self.location_r[resource][0] + r_padx, self.location_r[resource][1] + r_padyh), "HOLD")
                if requesters[resource][process]:
                    print("Resource %d being requested by process %d." % (resource, process))
                    self.draw_arrow((self.location_r[resource][0] + r_padx, self.location_r[resource][1] + r_padyr), (self.location_p[process][0], self.location_p[process][1] + p_padr), "REQUEST")
        if deadlocked_t[0]:
            self.status_text3 = self.font_menu_b.render("STATUS: DEADLOCKED", False, self.red)
            self.screen.blit(self.status_text3, (820, 220))
            p_xpad = -15
            p_ypad = -22
            print(deadlocked_t[1])
            for vertex in deadlocked_t[1]:
                ...
                self.render_text(self.font_pr, "p" + str(vertex),
                                 ...
                                 self.black)
            print(self.logic.get_connected_v())
            print(self.logic.get_request_edges())
        else:
